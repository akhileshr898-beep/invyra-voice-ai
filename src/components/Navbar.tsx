...
export function Navbar({
  ...
  return (
    <header className="sticky top-0 z-40 bg-slate-950/80 backdrop-blur-2xl border-b border-slate-800/80 shadow-[0_4px_30px_rgba(0,0,0,0.6)] transition-all">
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-3">
            <div className="relative group">
              <div className="w-10 h-10 rounded-2xl bg-gradient-to-tr from-indigo-600 via-violet-600 to-teal-400 flex items-center justify-center text-white shadow-lg shadow-indigo-500/25 transition group-hover:scale-105">
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="font-extrabold text-lg text-white tracking-tight">
                  Invyra<span className="text-indigo-400">.ai</span>
                </span>
                <span className="hidden sm:inline-flex items-center gap-1 text-[11px] font-semibold bg-indigo-500/10 text-indigo-300 px-2 py-0.5 rounded-full border border-indigo-500/30">
                  <Sparkles className="w-3 h-3 text-indigo-400" /> Voice Assistant
                </span>
              </div>
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center gap-2.5 px-3.5 py-1.5 rounded-xl bg-slate-900/90 hover:bg-slate-800 text-slate-100 text-xs sm:text-sm font-semibold transition border border-slate-700/80 shadow-inner active:scale-[0.98]"
            >
              ...
              <span className="max-w-[130px] sm:max-w-[210px] truncate text-slate-100">
            ...
            {dropdownOpen && (
              <div className="absolute right-0 mt-2.5 w-80 bg-slate-900/95 backdrop-blur-2xl rounded-2xl shadow-2xl border border-slate-800 py-2.5 z-50 animate-in fade-in slide-in-from-top-2 duration-150 ring-1 ring-white/10">
                <div className="px-4 py-1.5 text-[11px] font-bold uppercase tracking-wider text-slate-400 flex items-center justify-between">
                  ...
                  <span className="text-[10px] font-mono text-indigo-400">{businesses.length} active</span>
                </div>
                  ...
                  {businesses.map((biz) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          <div className={`w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 ${isSelected ? "bg-indigo-600 text-white shadow-sm" : "bg-slate-800 text-slate-400"}`}>
                            <Icon className="w-4 h-4" />
                          </div>
                          <div className="truncate flex-1">
                            <div className="truncate font-semibold text-slate-100">{biz.name}</div>
                        ...
                        <div className="flex items-center gap-1.5 flex-shrink-0">
                          {isSelected && (
                            <div className="w-2 h-2 rounded-full bg-emerald-400 mr-1 shadow-[0_0_8px_rgba(52,211,153,0.8)]" title="Currently Active"></div>
                          )}
                          {onDeleteBusiness && (
                            <button
                              onClick={(e) => {
                              ...
                              className={`p-1.5 rounded-lg transition ${
                                canDelete 
                                  ? "text-slate-500 hover:text-rose-400 hover:bg-rose-500/10 opacity-70 group-hover:opacity-100" 
                ...
                <div className="border-t border-slate-800 mt-2 pt-2 px-2">
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 py-2 px-3 rounded-xl text-xs font-semibold text-indigo-400 hover:bg-indigo-600/15 border border-indigo-500/20 transition active:scale-95"
                  >
            ...
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  {item.label}
                  {item.badge && (
                    <span className="ml-1 text-[9px] font-extrabold uppercase tracking-wider bg-emerald-500/20 text-emerald-300 border border-emerald-500/40 px-1.5 py-0.2 rounded-md">
                      {item.badge}
                    </span>
                  )}
                </button>
          ...
          {currentUser && (
            <div className="hidden lg:flex items-center gap-2 pl-2 border-l border-slate-800">
              <div className="flex items-center gap-2 px-2.5 py-1 rounded-xl bg-slate-900/90 border border-slate-800 text-xs">
                <div className="w-6 h-6 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 text-white flex items-center justify-center font-bold text-[10px] shadow-sm">
                  {currentUser.owner_name?.slice(0, 1) || "U"}
                </div>
                <div className="flex flex-col text-left max-w-[120px]">
                  <span className="font-semibold text-slate-200 truncate text-[11px] leading-tight">
              ...
              {onLogout && (
                <button
                  onClick={onLogout}
                  title="Sign Out"
                  className="p-1.5 rounded-xl text-slate-400 hover:text-rose-400 hover:bg-rose-500/10 border border-transparent hover:border-rose-500/20 transition active:scale-95"
                >
      ...
      {/* Mobile Menu Drawer */}
      {mobileMenuOpen && (
        <div className="md:hidden border-t border-slate-800 bg-slate-950/95 backdrop-blur-2xl px-4 pt-3 pb-5 space-y-1 shadow-2xl animate-in slide-in-from-top-3 duration-200">
          {navItems.map((item) => {
            const Icon = item.icon;
            const isActive = activeTab === item.id;
            return (
          ...
          {currentUser && (
            <div className="pt-3 mt-2 border-t border-slate-800">
              <div className="flex items-center justify-between px-3 py-2 bg-slate-900 rounded-xl mb-2 border border-slate-800">
                <div className="flex items-center gap-2.5 truncate">
                  ...
                  <div className="truncate">
                    <div className="text-xs font-semibold text-slate-200 truncate">{currentUser.owner_name}</div>
              ...
              {onLogout && (
                <button
                  onClick={onLogout}
                  className="w-full flex items-center justify-center gap-2 py-2.5 px-3 rounded-xl text-xs font-semibold text-rose-400 bg-rose-500/10 border border-rose-500/20 hover:bg-rose-500/20 transition"
                >
                  <LogOut className="w-4 h-4" />
