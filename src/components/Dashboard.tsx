...
export function Dashboard({ business, conversations, onRefresh }: DashboardProps) {
  ...
  const getUrgencyBadge = (priority: string) => {
    switch (priority) {
      case "urgent":
        return (
          <span className="inline-flex items-center gap-1.5 px-3 py-0.5 rounded-full text-xs font-bold bg-rose-100 text-rose-800 border border-rose-200 shadow-2xs">
      ...
      case "high":
        return (
          <span className="inline-flex items-center gap-1 px-3 py-0.5 rounded-full text-xs font-bold bg-amber-100 text-amber-800 border border-amber-200">
            High Priority
          </span>
        );
      default:
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-blue-600 via-blue-500 to-indigo-600 text-white rounded-3xl p-6 sm:p-7 shadow-lg shadow-blue-500/20 relative overflow-hidden flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <div className="relative z-10">
          ...
          <p className="text-xs text-blue-100 mt-1 max-w-2xl leading-relaxed">
            Real-time feed of caller conversations, structured extraction parameters, Gemini AI summaries, and automated triage follow-up states.
          </p>
        </div>
      ...
      {/* 4 KPI METRIC CARDS (CRISP WHITE CARDS) */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-3xl border border-slate-200/90 shadow-sm space-y-1">
          <div className="text-xs text-slate-500 font-bold uppercase tracking-wider">Total Handled</div>
          <div className="text-3xl font-extrabold text-slate-900">{totalCalls}</div>
          <div className="text-[11px] text-blue-600 font-semibold">Autonomous callbacks</div>
        </div>

        <div className="bg-white p-5 rounded-3xl border border-rose-200/90 shadow-sm space-y-1 bg-gradient-to-br from-white to-rose-50/30">
          <div className="text-xs text-rose-600 font-bold uppercase tracking-wider flex items-center gap-1">
            <AlertTriangle className="w-3.5 h-3.5 text-rose-500" />
            Urgent Alerts
          </div>
          <div className="text-3xl font-extrabold text-rose-600">{urgentCount}</div>
          <div className="text-[11px] text-rose-500 font-semibold">Condition rules triggered</div>
        </div>

        <div className="bg-white p-5 rounded-3xl border border-amber-200/90 shadow-sm space-y-1 bg-gradient-to-br from-white to-amber-50/30">
          ...
          <div className="text-[11px] text-amber-600 font-semibold">Requires follow-up</div>
        </div>

        <div className="bg-white p-5 rounded-3xl border border-emerald-200/90 shadow-sm space-y-1 bg-gradient-to-br from-white to-emerald-50/30">
          <div className="text-xs text-emerald-600 font-bold uppercase tracking-wider">Resolved</div>
          <div className="text-3xl font-extrabold text-emerald-600">{closedCount}</div>
          <div className="text-[11px] text-emerald-600 font-semibold">Completed & closed</div>
        </div>
      </div>

      {/* SEARCH & FILTER BAR (CRISP WHITE CARD) */}
      ...
      <div className="space-y-4">
        {filtered.length === 0 ? (
          <div className="bg-white rounded-3xl border border-slate-200/90 p-12 text-center text-slate-400 shadow-sm">
              ...
              >
                  ...
                  <div className="flex items-center gap-3.5">
                    ...
                    <div>
                      <div className="flex items-center gap-2.5 flex-wrap">
                        ...
                        <span className="text-xs font-mono text-slate-600 bg-slate-100 px-2 py-0.5 rounded-md">{record.caller_phone}</span>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
                  ...
                  <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200/70">
                    ...
                    <div className="mt-2 text-[11px] text-slate-500 font-medium">
                      Status: <span className="capitalize font-bold text-slate-700">{record.status}</span> &bull; Lang: <span className="uppercase font-bold text-blue-700">{record.language || "en"}</span>
                    </div>
                  </div>

                  <div className="bg-blue-50/40 p-4 rounded-2xl border border-blue-100 md:col-span-2 space-y-2">
                    <div className="flex items-center justify-between">
                      <div className="font-bold text-blue-700 uppercase tracking-wider text-[10px] flex items-center gap-1.5">
                    ...
                    <div className="pt-2 border-t border-blue-200/60 text-blue-700 font-bold text-[11px] flex items-center gap-1.5">
                  ...
                  <div className="bg-slate-50/80 p-3.5 rounded-2xl border border-slate-200/60">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {Object.entries(record.collected_data).map(([k, v]) => (
                        ...
                        >
                          <span className="text-slate-400 font-normal">{k}:</span> <strong className="text-slate-900">{String(v)}</strong>
                        </span>
                      ))}
                    </div>
                  </div>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-2">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 px-4 py-2 bg-slate-100 hover:bg-blue-50 hover:text-blue-700 text-slate-800 rounded-xl text-xs font-bold transition self-start sm:self-auto active:scale-95 border border-slate-200"
                  >
      ...
      {/* FULL TRANSCRIPT MODAL DIALOG (CRISP WHITE GLASS) */}
      {selectedRecord && (
          ...
          <div className="bg-white rounded-3xl max-w-2xl w-full max-h-[85vh] flex flex-col shadow-2xl border border-slate-200 animate-in fade-in zoom-in-95 duration-150">
            ...
            <div className="p-6 border-b border-slate-100 flex items-center justify-between">
              ...
              <button
                ...
                className="w-8 h-8 rounded-xl hover:bg-slate-100 text-slate-400 hover:text-slate-700 flex items-center justify-center font-bold text-sm"
              >
                ✕
              </button>
            </div>
                ...
                selectedRecord.transcript.map((msg, i) => {
                  ...
                  return (
                    <div key={i} className={`flex gap-3 ${isUser ? "justify-end" : "justify-start"}`}>
                      {!isUser && (
                        <div className="w-8 h-8 rounded-xl bg-gradient-to-tr from-blue-600 to-indigo-600 text-white flex items-center justify-center text-xs flex-shrink-0 mt-0.5 shadow-sm">
                          <Bot className="w-4 h-4" />
                        </div>
                      )}
                      <div className="max-w-[80%] space-y-1">
                        <div className={`p-4 rounded-3xl text-xs leading-relaxed ${
                          isUser ? "bg-blue-600 text-white font-medium" : "bg-white text-slate-800 border border-slate-200 shadow-2xs"
                        }`}>
                          <p>{msg.message}</p>
                          {msg.tool_call && (
            ...
            {/* Modal Footer */}
            <div className="p-5 border-t border-slate-100 bg-white flex items-center justify-between rounded-b-3xl">
              <div className="text-xs text-slate-500">
                Action: <strong className="text-slate-900">{selectedRecord.action_performed}</strong>
              </div>
              <button
                onClick={() => setSelectedRecord(null)}
                className="px-5 py-2 bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold rounded-xl shadow-xs active:scale-95 transition"
              >
                Close Transcript
              </button>
            </div>
